...
export function LobbyButton({ isLobby, toggleLobby }: { isLobby: boolean; toggleLobby: () => void }) {
  return (
    <motion.div 
      className="fixed top-4 left-4 sm:top-auto sm:left-auto sm:bottom-8 sm:right-16 z-[101] flex flex-col items-start sm:items-end"
      animate={{ y: [0, -10, 0] }}
      transition={{ 
        duration: 1.5, 
        repeat: Infinity, 
    ...
    >
      <div className="flex flex-col items-start sm:items-end scale-[0.65] sm:scale-100 origin-top-left sm:origin-bottom-right">
        ...
        >
           ...
           <div className="overflow-hidden h-10 sm:h-12 flex flex-col justify-center pr-2 sm:pr-4 relative">
              <span className="font-thick text-xl sm:text-2xl uppercase text-black leading-none tracking-widest md:tracking-normal group-hover:-translate-y-16 transition-transform duration-300">
